...
- **D. Leaving or Losing Connection:** Before the game starts, a player who loses their connection keeps their seat for two minutes (default) before losing it. Once the game has started, nobody loses their seat. A player who is away when they next have to act still gets the normal time limit (Section 6.C). If they are still away when it runs out, they sit out the rest of the round: their hand is revealed and scored, but they cannot win. A player who is away when a new round is dealt sits that round out and is dealt back in at the first deal after they return.
- **E. The Host Leaving:** Once the game has started, a host who leaves hands the host role to another player still at the table straight away. A host whose connection drops stays host for 30 seconds (default) and then hands it on the same way. If nobody else is at the table at that point, the first player to return becomes the host. A host who comes back after being replaced plays on as an ordinary player. Before the game starts, the host role moves to another player when the host gives up their seat.
