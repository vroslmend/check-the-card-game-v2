...
export function OptimizedShapes({ mouseX, mouseY, scrollY }: OptimizedShapesProps) {
  ...
  const shape1X = useTransform(smoothMouseX, [-1, 1], [-30, 30])
  const shape1Y = useTransform(smoothMouseY, [-1, 1], [-20, 20])

  const shape2X = useTransform(smoothMouseX, [-1, 1], [20, -20])
  const shape2Y = useTransform(smoothMouseY, [-1, 1], [15, -15])

  const shape3X = useTransform(smoothMouseX, [-1, 1], [-15, 15])
  const shape3Y = useTransform(smoothMouseY, [-1, 1], [-10, 10])
