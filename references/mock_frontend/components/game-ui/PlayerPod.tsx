...
export function PlayerPod({ player, isCurrentPlayer, position }: PlayerPodProps) {
  const statusConfig = {
    waiting: {
    ...
    >
      ...
      >
        ...
        <div className="space-y-1 text-center">
          ...
          <div className="flex items-center justify-center gap-1">
            <motion.div
              ...
              transition={{
                duration: player.status === "playing" ? 2 : 1,
                repeat: player.status === "playing" || player.status === "checking" ? Number.POSITIVE_INFINITY : 0,
              }}
            >
        ...
        <div className="relative">
          ...
          >
            {Array.from({ length: Math.min(player.handSize, 5) }, (_, i) => (
              <motion.div
                key={i}
                variants={{
              ...
              >
                <CardBack size="sm" layoutId={`opponent-${player.id}-card-${i}`} />
              </motion.div>
            ))}
          </motion.div>
        ...
        {/* Status Glow */}
        {(player.status === "checking" || player.status === "playing") && (
          <motion.div
            className={cn(
