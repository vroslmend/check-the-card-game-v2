...
export const uiMachine = setup({
  types: {
    ...
    emitted: {} as { type: 'EMIT_TO_SOCKET'; eventName: SocketEventName; payload: any },
  },
  actions: {
  ...
  guards: {
    ...
    canDrawFromDiscard: ({ context }) => {
      ...
      const topCard = context.currentGameState.discardPile[context.currentGameState.discardPile.length - 1];
      const isSpecialCard = ['K', 'Q', 'J'].includes(topCard.rank);
